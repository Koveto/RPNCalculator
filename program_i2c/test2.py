...
class Calculator:
    ...
    def update_display(self):
        ...
        if isinstance(x_value, complex):
            # Format complex number for display with scientific notation
            real_part = f"{x_value.real:.1e}".replace('+', '')
            imag_part = f"{x_value.imag:.1e}".replace('+', '')
            x_display = f"X: {real_part}+j{imag_part}"
            if len(x_display) > self.lcd.columns:
                x_display = f"X: {real_part[:8]}+j{imag_part[:6]}"
        else:
        ...
        if isinstance(y_value, complex):
            # Format complex number for display with scientific notation
            real_part = f"{y_value.real:.2e}".replace('+', '')
            imag_part = f"{y_value.imag:.2e}".replace('+', '')
            y_display = f"Y: {real_part} + j{imag_part}"
            if len(y_display) > self.lcd.columns:
                y_display = f"X: {real_part[:8]} + j{imag_part[:6]}"
        else:
            y_display = f"Y: {float(y_value):.4g}"  # Use general format for large/small numbers
